...
if(preg_match('/^(\d{2})-(\d{2})-(\d{4})$/', $object['date'], $inputDate)){
	$line='';
	$submit=array("week"=> date("W",strtotime($object['date'])),"year"=>$inputDate[3],'team'=>$_SESSION['team']);
	$request = $bdd->prepare("UPDATE `slotSchedule` AS s LEFT JOIN `roomusers` AS u ON u.id_user = s.ref_user SET s.valid='3' WHERE s.week=:week AND s.year=:year AND s.valid>0 AND s.valid<3 AND u.team=:team");
