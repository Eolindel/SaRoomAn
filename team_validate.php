...
if(isset($object['date']) AND preg_match('/^(\d{2})-(\d{2})-(\d{4})$/', $object['date'], $inputDate)){
	$line='';
		...
		$line.='</table>';
		$line.='<br><br><p class="warning">There is no turning back once this is done, so don\'t forget to check your team occupation as well as the room occupation.</p><form action="team_validate2.php" method="post">'.input_r('date', $object, 10,"hidden").'<input type="submit" value="Validate all these slots"></form><br>';


		$request = $bdd->prepare("SELECT u.nom, u.prenom, u.team, u.mail, u.statut, u.telephone, r.building, r.floor,r.officeName, s.date, s.week, s.start, s.end, s.length,s.valid FROM `slotSchedule` AS s LEFT JOIN `roomusers` AS u ON u.id_user = s.ref_user LEFT JOIN rooms AS r ON s.ref_room = r.id_room WHERE s.week=:week AND s.year=:year AND s.valid>=3 AND u.team=:team ORDER BY u.nom,s.day,s.start");
		$request->execute( array("week"=> date("W",strtotime($object['date'])),"year"=>$inputDate[3],'team'=>$_SESSION['team'] ) );	
